...
export const OutwardReportView: React.FC = () => {
  ...
  // Dynamic distinct target currencies present in filtered transactions
  const targetCurrenciesList = useMemo(() => {
    const list = Array.from(new Set(filteredTxs.map(tx => tx.targetCurrency || 'THB')));
    // Ensure common currencies show first if present
  ...
  // Day by Day Aggregation (User Request #3)
  ...
  // Grand Total for Day by Day Report
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2.5">
            <span className="w-9 h-9 rounded-xl bg-sky-500/20 text-sky-400 flex items-center justify-center">
          ...
          <p className="text-xs text-slate-400 mt-1">
            {language === 'my' 
              ? 'ပြည်တွင်း ပြည်ပ ငွေလွှဲပို့မှု အစီရင်ခံစာနှင့် ရက်အလိုက် စုစုပေါင်း စာရင်းဇယား' 
              : 'Detailed audit ledger and consolidated day-by-day outward remittance statements with currency breakdown'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center bg-slate-800 p-1 rounded-xl border border-slate-700">
            ...
            >
              ...
              <span className="text-black">{language === 'my' ? 'Total Outward Report (Day by Day)' : 'Day by Day Total Report'}</span>
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
          ...
          <span className="text-xs text-slate-500 mt-1 block">
            {filteredTxs.length} {language === 'my' ? 'စောင် ပါဝင်သည်' : 'transactions'} across {dayByDayTotals.length} {language === 'my' ? 'ရက်' : 'operating days'}
          </span>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
          <span className="text-xs text-slate-400 uppercase font-bold tracking-wider">Average Daily Volume</span>
          ...
          <span className="text-xs text-slate-500 mt-1 block">Avg per day ({dayByDayTotals.length} days active)</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div className="sm:col-span-2 md:col-span-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-9 pr-3 py-2 text-white placeholder-slate-500 focus:outline-none focus:border-sky-500 text-sm h-[38px]"
              />
            </div>
          </div>
          ...
          <div className="col-span-1">
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white font-mono text-sm focus:outline-none focus:border-sky-500 cursor-pointer h-[38px]"
          ...
          <div className="col-span-1">
            ...
            <select
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-sky-500 cursor-pointer text-sm h-[38px]"
      ...
      {/* VIEW 1: Total Outward Report (Day by Day with Currency Columns - User Request #3) */}
      {viewMode === 'DAY_BY_DAY' && (
          ...
          <div className="p-4 bg-slate-950/60 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-sm font-bold text-white">
                {language === 'my' ? 'ရက်အလိုက် စုစုပေါင်း အစီရင်ခံစာ (Day by Day Total with Currency Columns)' : 'Day by Day Outward Remittance Summary with Currency Columns'}
              </h3>
            </div>
            <span className="text-xs text-slate-400">
              Showing {dayByDayTotals.length} active day(s) | {filteredTxs.length} total transaction(s)
            </span>
          </div>
            ...
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950 text-slate-400 font-semibold uppercase tracking-wider border-b border-slate-800">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right">Send Total (MMK)</th>
                  {targetCurrenciesList.map(curr => (
                    <th key={curr} className="px-4 py-3 text-right text-emerald-400">
                      Total ({curr})
                    </th>
                  ))}
                  ...
                  dayByDayTotals.map((day) => (
                    <tr key={day.date} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-sky-500/20 text-sky-300 text-[11px] font-bold">
                          {day.txCount}
                        </span>
                      </td>
                      ...
                      {targetCurrenciesList.map(curr => {
                        ...
                        return (
                          <td key={curr} className="px-4 py-3 text-right text-emerald-300 font-semibold">
                      ...
                      <td className="px-4 py-3 text-right font-black text-sky-300 font-sans">
                        {day.totalVolumeMMK.toLocaleString()} <span className="text-[10px] text-sky-500 font-mono">MMK</span>
                      </td>
                    </tr>
                  ))
                )}
              ...
              {dayByDayTotals.length > 0 && (
                <tfoot className="bg-slate-950 font-mono font-bold text-white border-t-2 border-slate-700">
                  <tr>
                    <td className="px-4 py-3 font-sans uppercase tracking-wider text-sky-400">
                      GRAND TOTAL
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-sky-600 text-white text-xs">
                        {grandTotalSummary.txCount}
                      </span>
                    </td>
                    ...
                    {targetCurrenciesList.map(curr => {
                      ...
                      return (
                        <td key={curr} className="px-4 py-3 text-right text-emerald-300 text-xs">
                    ...
                    <td className="px-4 py-3 text-right text-sm text-sky-300 font-black">
                      {grandTotalSummary.totalVolumeMMK.toLocaleString()} <span className="text-xs text-sky-400">MMK</span>
                    </td>
                  </tr>
                </tfoot>
              )}
                    ...
                    <tr key={tx.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="font-medium text-slate-200 text-[11px] flex items-center space-x-1">
                          <UserIcon className="w-3 h-3 text-sky-400 shrink-0" />
