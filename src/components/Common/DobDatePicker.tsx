    ...
    <div className={`relative ${className}`} ref={containerRef}>
      {label && (
        <div className="flex items-center justify-between mb-1">
          <label htmlFor={id} className="block text-slate-700 font-semibold text-xs">
      ...
      <div className="relative flex items-center rounded-lg border border-slate-300 bg-white focus-within:border-sky-500 focus-within:ring-1 focus-within:ring-sky-500 transition-all shadow-xs">
        <input
          id={id}
          type="text"
          value={inputValue}
          ...
          disabled={disabled}
          required={required}
          className="w-full bg-transparent px-3 py-2 text-slate-900 font-mono text-xs focus:outline-none placeholder:text-slate-400 tracking-wider"
        />
        ...
        {inputValue && !disabled && (
          <button
            ...
            className="p-1 text-slate-400 hover:text-slate-600 transition-colors mr-0.5 cursor-pointer"
        ...
        <button
          ...
          disabled={disabled}
          className="px-2.5 py-2 text-sky-600 hover:text-sky-700 hover:bg-slate-50 rounded-r-lg transition-colors border-l border-slate-200 flex items-center justify-center cursor-pointer shrink-0"
      ...
      <div className="flex items-center justify-between mt-1 text-[10px] text-slate-500 font-mono px-1">
        <span>Format: <strong className="text-slate-700">DD/MM/YYYY</strong> (e.g. 25/08/1992)</span>
        <button
          type="button"
          onClick={triggerNativePicker}
          className="text-sky-600 hover:text-sky-700 underline underline-offset-2 hover:no-underline cursor-pointer"
        >
      ...
      {isOpen && (
        <div className="absolute left-0 sm:right-0 sm:left-auto top-full mt-1 z-50 w-72 sm:w-80 bg-white border border-slate-200 rounded-xl shadow-2xl p-4 text-slate-900 animate-in fade-in zoom-in-95 duration-150">
          
          {/* Calendar Header with Month and Year Selectors */}
          <div className="flex items-center justify-between gap-1 mb-3 pb-2 border-b border-slate-200">
            <button
              type="button"
              onClick={prevMonth}
            ...
            <div className="flex items-center space-x-1.5 flex-1 justify-center">
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-300 rounded-md px-2 py-1 text-xs font-semibold text-slate-800 focus:outline-none focus:border-sky-500 cursor-pointer"
              >
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-300 rounded-md px-2 py-1 text-xs font-mono font-bold text-amber-700 focus:outline-none focus:border-sky-500 cursor-pointer max-h-48"
              >
                {yearsList.map(y => (
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-[10px] font-bold text-slate-500 mb-1">
            <span className="text-rose-600">Su</span>
            <span>Mo</span>
            <span>Tu</span>
            <span>We</span>
            <span>Th</span>
            <span>Fr</span>
            <span className="text-sky-600">Sa</span>
          </div>
          ...
          <div className="mt-3 pt-2.5 border-t border-slate-200 flex items-center justify-between text-[11px]">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className="px-2.5 py-1 rounded-md bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold cursor-pointer"
              >
